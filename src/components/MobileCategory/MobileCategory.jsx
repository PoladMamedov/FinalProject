...
const MobileCategory = ({ category, setIsMenuOpen }) => {
   ...
   return (
      <>
         <NavLink className="header__mobile-category" to={`/${category.replace("Smart watches", "watches")}`}>
            <li onClick={(e) => hanldeLinkClick(e)} className="header__mobile-category">
               {category}
               <svg className={`header__mobile-category-btn${isCategoryOpen ? "--open" : ""}`} xmlns="http://www.w3.org/2000/svg" height="1.1em" style={{ fill: "#393d45" }} viewBox="0 0 384 512">
